...
describe('AgentConfigManager', () => {
    ...
    test('should update lastUsed timestamp on reuse', async () => {
        ...
        assert.strictEqual(new Date(secondLastUsed) > new Date(firstLastUsed), true, 'secondLastUsed should be later');
    });
    test('should throw error when updating non-existent agent name', async () => {
        ...
        }, /Agent .* not found/, 'Should throw error for non-existent agent');
    });
...
//# sourceMappingURL=agent-config-manager.test.js.map