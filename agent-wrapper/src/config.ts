...
const ConfigSchema = z
  ...
  .refine(
    data => {
      ...
      // For Claude subscription auth, apiKey is not required
      return true;
    },
    {
...
export function loadConfig(overrides: Partial<Config> = {}): Config {
  const envConfig = {
    ...
    authType: process.env['ONSEMBL_AUTH_TYPE'] as
      | 'api-key'
      | 'subscription'
      | undefined,
    ...
    // Process limits
    maxMemoryMb: process.env['ONSEMBL_MAX_MEMORY_MB']
      ? parseInt(process.env['ONSEMBL_MAX_MEMORY_MB'], 10)
      : undefined,
    maxCpuPercent: process.env['ONSEMBL_MAX_CPU_PERCENT']
      ? parseInt(process.env['ONSEMBL_MAX_CPU_PERCENT'], 10)
      : undefined,

    // Connection settings
    reconnectAttempts: process.env['ONSEMBL_RECONNECT_ATTEMPTS']
      ? parseInt(process.env['ONSEMBL_RECONNECT_ATTEMPTS'], 10)
      : undefined,
    reconnectBaseDelay: process.env['ONSEMBL_RECONNECT_BASE_DELAY']
      ? parseInt(process.env['ONSEMBL_RECONNECT_BASE_DELAY'], 10)
      : undefined,
    heartbeatInterval: process.env['ONSEMBL_HEARTBEAT_INTERVAL']
      ? parseInt(process.env['ONSEMBL_HEARTBEAT_INTERVAL'], 10)
      : undefined,

    // Output settings
    outputBufferSize: process.env['ONSEMBL_OUTPUT_BUFFER_SIZE']
      ? parseInt(process.env['ONSEMBL_OUTPUT_BUFFER_SIZE'], 10)
      : undefined,
    outputFlushInterval: process.env['ONSEMBL_OUTPUT_FLUSH_INTERVAL']
      ? parseInt(process.env['ONSEMBL_OUTPUT_FLUSH_INTERVAL'], 10)
      : undefined,
    ...
    codex: {
      ...
      maxTokens: process.env['CODEX_MAX_TOKENS']
        ? parseInt(process.env['CODEX_MAX_TOKENS'], 10)
        : undefined,
      temperature: process.env['CODEX_TEMPERATURE']
        ? parseFloat(process.env['CODEX_TEMPERATURE'])
        : undefined,
    },
  };
  ...
  if (!validatedConfig.agentCommand) {
    validatedConfig.agentCommand =
      DEFAULT_AGENT_COMMANDS[validatedConfig.agentType];
  }
