...
describe('Command Processor', () => {
    ...
    describe('Initialization', () => {
        it('should initialize worker with correct configuration', () => {
            expect(MockedWorker).toHaveBeenCalledWith(mockConfig.queue.name, expect.any(Function), // processor function
            {
    ...
    describe('Command Processing', () => {
        ...
        beforeEach(() => {
            ...
            processorFunction = workerCall[1];
        });
    ...
    describe('Worker Event Handling', () => {
        it('should handle job completion events', () => {
            ...
            expect(mockLogger.info).toHaveBeenCalledWith({ jobId: 'job-1', jobName: 'test-job', result: mockResult }, 'Job completed');
        });
        it('should handle job failure events', () => {
            ...
            expect(mockLogger.error).toHaveBeenCalledWith({ jobId: 'job-1', jobName: 'test-job', error: mockError }, 'Job failed');
        });
        it('should handle stalled job events', () => {
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith({ jobId: 'job-1', jobName: 'test-job' }, 'Job stalled');
        });
        it('should handle worker error events', () => {
            ...
            expect(mockLogger.error).toHaveBeenCalledWith({ error: mockError }, 'Worker error');
        });
    });
    describe('Command Type Handlers', () => {
        ...
        it('should handle unknown command types', async () => {
            const command = createMockCommand({ type: 'unknown' });
            const mockJob = {
                id: 'job-1',
                data: { command, userId: 'user-1', agentId: 'agent-1', priority: 50 },
                updateProgress: jest.fn().mockResolvedValue(undefined)
    ...
    describe('Error Recovery', () => {
        it('should handle worker initialization failure', () => {
            ...
            expect(() => new CommandProcessor(mockLogger)).toThrow('Worker initialization failed');
        });
        it('should handle job processing timeout', async () => {
            const processorFunction = MockedWorker.mock.calls[0][1];
            const command = createMockCommand({
                type: 'execute',
            ...
            jest.spyOn(processor, 'executeCommand').mockImplementation(() => new Promise(resolve => setTimeout(resolve, 200)));
            const result = await processorFunction(mockJob);
            expect(result.success).toBe(false);
            expect(result.error).toContain('timeout');
        });
        it('should handle progress update failures', async () => {
            ...
            expect(mockLogger.error).toHaveBeenCalledWith(expect.objectContaining({ error: expect.any(Error) }), 'Failed to update job progress');
        });
    });
