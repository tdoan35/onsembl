...
describe('Redis Connection Management', () => {
  ...
  describe('createRedisConnection', () => {
    ...
    it('should handle connection events correctly', () => {
      ...
      const connectHandler = onCalls.find(call => call[0] === 'connect')?.[1] as Function;
      connectHandler?.();
      expect(consoleSpy).toHaveBeenCalledWith('Redis connecting...');

      // Trigger ready event
      const readyHandler = onCalls.find(call => call[0] === 'ready')?.[1] as Function;
      readyHandler?.();
      expect(consoleSpy).toHaveBeenCalledWith('Redis connection ready');

      // Trigger error event
      const errorHandler = onCalls.find(call => call[0] === 'error')?.[1] as Function;
      const testError = new Error('Connection failed');
      errorHandler?.(testError);
      expect(consoleErrorSpy).toHaveBeenCalledWith('Redis connection error:', testError);

      // Trigger close event
      const closeHandler = onCalls.find(call => call[0] === 'close')?.[1] as Function;
      closeHandler?.();
      expect(consoleSpy).toHaveBeenCalledWith('Redis connection closed');

      // Trigger reconnecting event
      const reconnectingHandler = onCalls.find(call => call[0] === 'reconnecting')?.[1] as Function;
      reconnectingHandler?.();
  ...
  describe('Error Handling', () => {
    ...
    it('should handle configuration errors gracefully', async () => {
      const { ConfigManager } = await import('../src/config.js');
      (ConfigManager.getRedisOptions as jest.Mock).mockImplementation(() => {
  ...
  describe('Singleton Reset', () => {
    it('should allow singleton reset in test environments', () => {
      ...
      (global as any).__resetRedisClient = () => {
        (getRedisClient as any).instance = null;
      };

      if ((global as any).__resetRedisClient) {
        (global as any).__resetRedisClient();
      }
