...
import type { QueuedCommand, CommandJobData, InterruptRequest } from '../src/types.js';
...
describe('Command Queue Integration', () => {
  ...
  describe('Error Handling and Recovery', () => {
    ...
    it('should enforce queue size limits', async () => {
      ...
      await expect(
        smallQueueManager.addCommand(command3, { userId: 'user-1' })
  ...
  describe('Performance and Scalability', () => {
    it('should handle multiple concurrent commands', async () => {
      ...
      const commands = Array.from({ length: 10 }, (_, i) =>
        createMockCommand({ id: `cmd-${i}`, content: `task ${i}` })
      );

      // Add all commands concurrently
      const jobs = await Promise.all(
        commands.map(cmd =>
          queueManager.addCommand(cmd, {
            userId: 'test-user',
  ...
  describe('System Monitoring and Health', () => {
    ...
    it('should handle graceful shutdown of all components', async () => {
      ...
      await expect(
        queueManager.addCommand(command, { userId: 'post-shutdown' })
