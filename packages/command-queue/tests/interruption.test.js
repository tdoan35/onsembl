...
describe('Command Interruption Logic', () => {
    ...
    describe('Waiting Job Interruption', () => {
        ...
        it('should handle waiting job removal failure', async () => {
            ...
            expect(mockLogger.error).toHaveBeenCalledWith({ error: expect.any(Error), commandId: 'test-cmd-1' }, 'Failed to interrupt command');
        });
    });
    describe('Active Job Interruption', () => {
        ...
        it('should force interrupt active job on timeout', async () => {
            const mockJob = {
                ...
                isWaiting: jest.fn().mockResolvedValue(false),
                updateData: jest.fn().mockResolvedValue(undefined),
                waitUntilFinished: jest.fn().mockImplementation(() => new Promise(resolve => setTimeout(resolve, 10000)) // Takes longer than timeout
                ),
                remove: jest.fn().mockResolvedValue(undefined)
            };
            mockQueue.getJobs.mockResolvedValue([mockJob]);
        ...
        it('should use default timeout when not specified', async () => {
            const mockJob = {
                ...
                isWaiting: jest.fn().mockResolvedValue(false),
                updateData: jest.fn().mockResolvedValue(undefined),
                waitUntilFinished: jest.fn().mockImplementation(() => new Promise(resolve => setTimeout(resolve, 6000)) // Longer than default timeout
                ),
                remove: jest.fn().mockResolvedValue(undefined)
            };
            mockQueue.getJobs.mockResolvedValue([mockJob]);
    ...
    describe('Multiple Interruption Scenarios', () => {
        it('should handle multiple concurrent interruptions', async () => {
            ...
            const results = await Promise.all(requests.map(req => queueManager.interruptCommand(req)));
...
//# sourceMappingURL=interruption.test.js.map