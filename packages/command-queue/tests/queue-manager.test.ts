...
import type { QueuedCommand, CommandJobData, QueueMetrics } from '../src/types.js';
...
describe('Queue Manager Operations', () => {
  ...
  describe('Initialization', () => {
    it('should initialize queue successfully', async () => {
      ...
      expect(MockedQueueEvents).toHaveBeenCalledWith(
        mockConfig.queue.name,
        expect.objectContaining({
          connection: expect.any(Object)
        })
    ...
    it('should handle initialization timeout', async () => {
      ...
      mockQueue.waitUntilReady.mockImplementation(() =>
  ...
  describe('Queue Operations', () => {
    ...
    it('should clean old jobs', async () => {
      ...
      expect(mockLogger.info).toHaveBeenCalledWith(
        { completedCleaned: ['job1', 'job2'], failedCleaned: ['job3'] },
        'Queue cleaned'
      );
    });
  });
  ...
  describe('Metrics', () => {
    it('should calculate comprehensive queue metrics', async () => {
      ...
      const expectedMetrics: QueueMetrics = {
  ...
  describe('Error Handling', () => {
    it('should prevent operations when not initialized', async () => {
      ...
      await expect(uninitializedManager.addCommand({} as any, {} as any))
    ...
    it('should prevent operations during shutdown', async () => {
      ...
      await expect(queueManager.addCommand({} as any, {} as any))
    ...
    it('should handle queue errors', () => {
      ...
      const queueErrorHandler = mockQueue.on.mock.calls.find(
        call => call[0] === 'error'
      )?.[1];
