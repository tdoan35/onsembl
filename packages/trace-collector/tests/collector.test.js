...
class MockTraceStorage {
    ...
    async search(filter, limit = 50, offset = 0) {
        ...
        if (filter.dateRange) {
            results = results.filter(t => t.startedAt >= filter.dateRange.from && t.startedAt <= filter.dateRange.to);
        }
        if (filter.durationRange) {
            results = results.filter(t => (t.durationMs || 0) >= filter.durationRange.min &&
...
describe('TraceCollector', () => {
    ...
    describe('addTrace', () => {
        it('should add trace successfully', async () => {
            ...
            const traceAddedPromise = new Promise((resolve) => {
        ...
        it('should calculate trace depth correctly', async () => {
            ...
            await collector.addTrace(childTrace);
            const childAddedPromise = new Promise((resolve) => {
                collector.once('trace:added', resolve);
            });
        ...
        it('should enforce max traces per command limit', async () => {
            const config = { maxTracesPerCommand: 3 };
            const limitedCollector = new TraceCollector(storage, logger, config);
            const traces = [
    ...
    describe('updateTrace', () => {
        ...
        it('should preserve trace ID during update', async () => {
            const trace = mockTrace();
            await collector.addTrace(trace);
            await collector.updateTrace(trace.id, { id: 'different-id' });
            const updatedTrace = await storage.get(trace.id);
    ...
    describe('getCommandAggregation', () => {
        ...
        it('should include performance analysis when enabled', async () => {
            const config = { enableRealTimeAnalysis: true };
    ...
    describe('getAgentStats', () => {
        ...
        it('should respect limit parameter', async () => {
            const traces = Array.from({ length: 10 }, (_, i) => mockTrace({ id: `trace-${i}` }));
            for (const trace of traces) {
                await collector.addTrace(trace);
            }
    ...
    describe('cleanup', () => {
        ...
        it('should return count of cleaned traces', async () => {
            const oldTraces = Array.from({ length: 5 }, (_, i) => mockTrace({ id: `old-${i}`, startedAt: Date.now() - 100000000 }));
