...
describe('TraceAggregator', () => {
    ...
    describe('aggregateTrees', () => {
        ...
        it('should include hot paths when enabled', () => {
            const options = { calculateHotPaths: true };
            const customAggregator = new TraceAggregator(logger, options);
        ...
        it('should include type groupings when enabled', () => {
            const options = { groupByType: true };
            const customAggregator = new TraceAggregator(logger, options);
            const traces = [
    ...
    describe('calculateTrends', () => {
        it('should return stable trends with insufficient data', () => {
            const historicalResults = [
                { timestamp: Date.now(), result: { metrics: { averageDuration: 1000, errorRate: 0.1, throughputPerSecond: 10, averageTokens: 100 } } }
            ];
        ...
        it('should detect improving trends', () => {
            const historicalResults = [
                // Older data
                { timestamp: Date.now() - 300000, result: { metrics: { averageDuration: 2000, errorRate: 0.2, throughputPerSecond: 5, averageTokens: 200 } } },
                { timestamp: Date.now() - 240000, result: { metrics: { averageDuration: 1900, errorRate: 0.18, throughputPerSecond: 6, averageTokens: 190 } } },
                { timestamp: Date.now() - 180000, result: { metrics: { averageDuration: 1800, errorRate: 0.16, throughputPerSecond: 7, averageTokens: 180 } } },
                // Recent data (improved)
                { timestamp: Date.now() - 120000, result: { metrics: { averageDuration: 1000, errorRate: 0.05, throughputPerSecond: 15, averageTokens: 100 } } },
                { timestamp: Date.now() - 60000, result: { metrics: { averageDuration: 900, errorRate: 0.04, throughputPerSecond: 16, averageTokens: 90 } } },
                { timestamp: Date.now(), result: { metrics: { averageDuration: 800, errorRate: 0.03, throughputPerSecond: 17, averageTokens: 80 } } }
            ];
            const trends = aggregator.calculateTrends(historicalResults);
        ...
        it('should detect degrading trends', () => {
            const historicalResults = [
                // Older data (better)
                { timestamp: Date.now() - 300000, result: { metrics: { averageDuration: 800, errorRate: 0.02, throughputPerSecond: 20, averageTokens: 80 } } },
                { timestamp: Date.now() - 240000, result: { metrics: { averageDuration: 850, errorRate: 0.03, throughputPerSecond: 19, averageTokens: 85 } } },
                { timestamp: Date.now() - 180000, result: { metrics: { averageDuration: 900, errorRate: 0.04, throughputPerSecond: 18, averageTokens: 90 } } },
                // Recent data (degraded)
                { timestamp: Date.now() - 120000, result: { metrics: { averageDuration: 1500, errorRate: 0.15, throughputPerSecond: 8, averageTokens: 150 } } },
                { timestamp: Date.now() - 60000, result: { metrics: { averageDuration: 1600, errorRate: 0.16, throughputPerSecond: 7, averageTokens: 160 } } },
                { timestamp: Date.now(), result: { metrics: { averageDuration: 1700, errorRate: 0.17, throughputPerSecond: 6, averageTokens: 170 } } }
            ];
            const trends = aggregator.calculateTrends(historicalResults);
    ...
    describe('edge cases', () => {
        ...
        it('should handle very large trace trees', () => {
            ...
            const traces = Array.from({ length: 1000 }, (_, i) => mockTrace({
                id: `trace-${i}`,
    ...
    describe('time window aggregation', () => {
        it('should create time windows when enabled', () => {
            const options = { timeWindowMs: 10000 }; // 10 second windows
            const customAggregator = new TraceAggregator(logger, options);
            const baseTime = Date.now();
            const traces = [
